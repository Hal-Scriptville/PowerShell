...
Updates made to modernize script and add other customizations.
...
2.A device compliance policy for: Windows10, Windows 11, iOS/ipad, Android for Work, Android Device Owner, macOS
3.A Windows 10 Endpoint Protection Profile for Windows Devices for Silent Bitlocker Encryption
4.M365 Apps to Windows 10 and MacOS
...
9.Windows Update Rings for Windows 10 and Windows 11
...
function Get-AuthToken {
    ...
    if ($null -eq (Get-MgUser -Top 1 -ErrorAction SilentlyContinue)) {
...
# Function to handle errors from Microsoft Graph
function Handle-GraphError {
    param (
        [Parameter(Mandatory = $true)]
    ...
    if ($null -ne $ErrorRecord.Exception -and $null -ne $ErrorRecord.Exception.Response) {
        try {
            $errorResponse = $ErrorRecord.Exception.Response.GetResponseStream()
            if ($null -ne $errorResponse) {
                $reader = New-Object System.IO.StreamReader($errorResponse)
                $responseBody = $reader.ReadToEnd()
                Write-Host "Error response: $responseBody"
            } else {
                Write-Host "Error response stream is null."
            }
        } catch {
            Write-Host "Error handling failed: $_"
        }
    } else {
        Write-Host "No response available for this error."
    }
}

####################################################

function Validate-AADGroup {
    param (
        [Parameter(Mandatory = $true)]
        [string] $GroupName
    )
    ...
    if ($null -eq $group) {
        throw "AAD Group - '$GroupName' doesn't exist, please specify a valid AAD Group."
    }
    return $group
}
...
function Add-TermsAndConditions {
    ...
    try {
        Write-Host "Starting Add-TermsAndConditions function"
        $terms = $jsonContent | ConvertFrom-Json

        # Log the converted terms object
        Write-Host "Converted JSON Content: " -ForegroundColor Yellow
        $terms | ConvertTo-Json -Depth 3 | Write-Host

        $url = "https://graph.microsoft.com/beta/deviceManagement/termsAndConditions"

        # Convert back to JSON string
        $termsJson = $terms | ConvertTo-Json -Depth 3
        $response = Invoke-MgGraphRequest -Uri $url -Method POST -Body $termsJson -ContentType "application/json"
...
function Add-DeviceCompliancePolicy {
    ...
    try {
        Write-Host "Creating Device Compliance Policy from JSON content..."

        # Convert JSON content to a PowerShell object
        $policyData = $jsonContent | ConvertFrom-Json

        # Log the converted JSON object for debugging
        Write-Host "Converted JSON Content: " -ForegroundColor Yellow
        $policyData | ConvertTo-Json -Depth 3 | Write-Host

        # Create the device compliance policy using the New-MgDeviceManagementDeviceCompliancePolicy cmdlet
        $policy = New-MgDeviceManagementDeviceCompliancePolicy -BodyParameter $policyData

        Write-Host "Device Compliance Policy created: $($policy.displayName)"
        return $policy
    } catch {
        Write-Host "Error: $_"
    }
...
function Add-DeviceCompliancePolicy {
    ...
    try {
        ...
        # Log the converted JSON object for debugging
        Write-Host "Converted JSON Content: " -ForegroundColor Yellow
        $policyData | ConvertTo-Json -Depth 5 | Write-Host
...
$CreateResult_macOS = Add-DeviceCompliancePolicy -jsonContent $JSON_macOS
...
####################################################
Function Add-WindowsUpdatePolicy(){

<#
.SYNOPSIS
This function is used to add a Windows update (ring) policy using the Graph API REST interface
.DESCRIPTION
The function connects to the Graph API Interface and adds an update ring configuration policy
.EXAMPLE
Add-WindowsUpdatePolicy -JSON $JSON
Adds a device update ring policy in Intune
.NOTES
NAME: Add-WindowsUpdatePolicy
#>
...
Write-Verbose "Resource: $DCP_resource"
    ...
    try {
        ...
        write-host "No JSON specified, please specify valid JSON for the Windows Update Policy..." -f Red
...
#########################################################
<#
Function Add-WindowsUpdatePolicyAssignment(){
#>
<#
.SYNOPSIS
This function is used to add a Windows Update configuration policy assignment using the Graph API REST interface
.DESCRIPTION
The function connects to the Graph API Interface and adds a Windows Update configuration policy assignment
.EXAMPLE
Add-WindowsUpdatePolicyAssignment -ConfigurationPolicyId $ConfigurationPolicyId -TargetGroupId $TargetGroupId
Adds a device Windows Update policy assignment in Intune
.NOTES
NAME: Add-WindowsUpdatePolicyAssignment
#>
<#
[cmdletbinding()]
...
$Resource = "deviceAppManagement/windowsManagedAppProtections"
    ...
    try {
        ...
        Test-JSON -JSON $JSON
		write-host "Adding JSON information to Windows Information Protection Policy: $JSON"
...
Function Add-WindowsBaselineSecurity(){

<#
.SYNOPSIS
This function is used to add a Windows Baseline Security policy using the Graph API REST interface
.DESCRIPTION
The function connects to the Graph API Interface and adds a Windows Baseline Security policy
.EXAMPLE
Add-WindowsBaselineSecurity -JSON $JSON
Adds a Windows Baseline Security policy in Intune
.NOTES
NAME: Add-WindowsBaselineSecurity
#>
...
$Resource = "deviceManagement/intents"
    ...
    try {
        ...
        write-host "No JSON specified, please specify valid JSON for the Baseline Security Policy..." -f Red
...
function Get-EnterpriseDomain {
	...
	write-host "Calling Get-EnterpriseDomain function" -ForegroundColor cyan
    try {
        $domain = Get-MgDomain -Filter "id eq '$domainName'" -ErrorAction Stop
        if ($null -eq $domain) {
...
function Get-TenantName {
    write-host "Calling Get-TenantName function" -ForegroundColor cyan
	try {
...
####################################################
<#	Configurations using JSON Files				#>
####################################################

$JSON_10QualityUpdate = @"
{
	...
	"description":"",
	"displayName":"Windows 10 Update Ring",
	"version":1,
...
$JSON_Windows10 = @"
  ...
  "osMinimumVersion": "10.0.19045.4170",
  "osMaximumVersion": null,
...
$JSON_Windows11 = @"
  ...
  "@odata.type": "#microsoft.graph.windows10CompliancePolicy",
  "description": "1. Require BitLocker\n2. Require Secure Boot to be enabled on the device\n3. Require code integrity\n4. Require Encryption of data storage on device.\n5. Firewall, Antivirus, Antispyware",
  "displayName": "Windows 11 Compliance Policy",
  "version": 7,
  "scheduledActionsForRule":[{"ruleName":"PasswordRequired","scheduledActionConfigurations":[{"actionType":"block","gracePeriodHours":0,"notificationTemplateId":""}]}],
  "passwordRequired": false,
  "passwordBlockSimple": false,
  ...
  "passwordPreviousPasswordBlockCount": null,
  "requireHealthyDeviceReport": false,
  "osMinimumVersion": "10.0.22631.3296",
  "osMaximumVersion": null,
  "mobileOsMinimumVersion": null,
  "mobileOsMaximumVersion": null,
  "earlyLaunchAntiMalwareDriverEnabled": false,
...
$JSON1 = @"
{
  ...
  "autoAcceptEula": true,
  "description":"Office 365 - Assigned",
  "developer": "Microsoft",
  "displayName":"Office 365 - Assigned",
  "excludedApps": {
    "groove": true,
    "infoPath": true,
...
####################################################
$ManagedAppPolicy_Windows = @"
{
	"displayName":"Windows Browser App Protection Worked",
	"description":"",
	"roleScopeTagIds":["0"],
	"version":"\"9c005f0e-0000-0100-0000-65fdb1ab0000\"",
	"isAssigned":false,
	"deployedAppCount":1,
	"printBlocked":true,
...
$group = Validate-AADGroup -GroupName $AADGroup
...
Write-Host
Write-Host "Starting Assign-TermsAndConditions Function" -ForegroundColor cyan
        if ($CreateResult.id) {
            $Assign_Policy = New-MgBetaDeviceManagementTermAndConditionGroupAssignment -TermsAndConditionsId $CreateResult.id -TargetGroupId $group.id
            Write-Host "Assigned '$($group.displayName)' with Group ID of '$($group.id)' to Customer Terms and Conditions/$($CreateResult.id)"
        } else {
            Write-Host "No Terms and Conditions ID was returned, unable to assign."
        }
...
Write-Host "Adding Windows 10 Compliance Policy from JSON..." -ForegroundColor Yellow

$CreateResult_Windows = Add-DeviceCompliancePolicy -JSON $JSON_Windows10
...
Write-Host "Adding Windows 11 Compliance Policy from JSON..." -ForegroundColor Yellow

$CreateResult_Windows = Add-DeviceCompliancePolicy -JSON $JSON_Windows11
...
Write-Host "Adding Windows 10 Update Policy from JSON..." -ForegroundColor Yellow

$CreateResult_UpdateProfile = Add-WindowsUpdatePolicy -JSON $JSON_10QualityUpdate

Write-Host "Windows 10 Update Policy created as" $CreateResult_UpdateProfile.id
write-host
Write-Host "Adding Windows 11 Update Policy from JSON..." -ForegroundColor Yellow

$CreateResult_UpdateProfile = Add-WindowsUpdatePolicy -JSON $JSON_11QualityUpdate

Write-Host "Windows 11 Update Policy created as" $CreateResult_UpdateProfile.id
write-host
...
<#
Write-Host "Adding Windows Baseline Security Policy from JSON..." -ForegroundColor Yellow

$CreateResult = Add-WindowsBaselineSecurity -JSON $BaselineSecurity_Windows

write-host "Policy created with id" $CreateResult.id

$WinBaselineSec_PolicyID = $CreateResult.id
...
Write-Host "Adding Windows Edge Information Protection Without Enrollment Policy from JSON..." -ForegroundColor Yellow

$CreateResult = Add-WindowsInformationProtectionPolicy -JSON $ManagedAppPolicy_Windows
...
$block = @"
 ...
 __    __  .______    _______       ___   .___________. _______  _______     .______   ____    ____     __    __       ___       __                 
|  |  |  | |   _  \  |       \     /   \  |           ||   ____||       \    |   _  \  \   \  /   /    |  |  |  |     /   \     |  |                
|  |  |  | |  |_)  | |  .--.  |   /  ^  \ `---|  |----`|  |__   |  .--.  |   |  |_)  |  \   \/   /     |  |__|  |    /  ^  \    |  |         ______ 
|  |  |  | |   ___/  |  |  |  |  /  /_\  \    |  |     |   __|  |  |  |  |   |   _  <    \_    _/      |   __   |   /  /_\  \   |  |        |______|
|  `--'  | |  |      |  '--'  | /  _____  \   |  |     |  |____ |  '--'  |   |  |_)  |     |  |        |  |  |  |  /  _____  \  |  `----.           
 \______/  | _|      |_______/ /__/     \__\  |__|     |_______||_______/    |______/      |__|        |__|  |__| /__/     \__\ |_______|           
                                                                                                                                                    
.___________. __    __       ___      .__   __.  __  ___      _______.       .___________..___  ___.  __  .__   __.  __    __       _______. __     
|           ||  |  |  |     /   \     |  \ |  | |  |/  /     /       |       |           ||   \/   | |  | |  \ |  | |  |  |  |     /       ||  |    
`---|  |----`|  |__|  |    /  ^  \    |   \|  | |  '  /     |   (----`       `---|  |----`|  \  /  | |  | |   \|  | |  |  |  |    |   (----`|  |    
    |  |     |   __   |   /  /_\  \   |  . `  | |    <       \   \               |  |     |  |\/|  | |  | |  . `  | |  |  |  |     \   \    |  |    
    |  |     |  |  |  |  /  _____  \  |  |\   | |  .  \  .----)   |    __        |  |     |  |  |  | |  | |  |\   | |  `--'  | .----)   |   |__|    
    |__|     |__|  |__| /__/     \__\ |__| \__| |__|\__\ |_______/    (_ )       |__|     |__|  |__| |__| |__| \__|  \______/  |_______/    (__)    
                                                                       |/                                                                           
